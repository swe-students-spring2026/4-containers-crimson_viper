...
def create_entry(username, date, entry_data):
    ...
    entry_data["created_at"] = (
        entry_data.get("created_at") or datetime.utcnow().isoformat()
    )
...
def update_entry(username, date, entry_index, updated_data):
    ...
    if (
        not day
        or "journal_entries" not in day
        or entry_index >= len(day["journal_entries"])
    ):
        return 0
    ...
    result = entries_collection.update_one(
        {"username": username, "date": date}, {"$set": {key: merged_entry}}
    )
...
def delete_entry(username, date, entry_index):
    ...
    result = entries_collection.update_one(
        {"username": username, "date": date}, {"$set": {"journal_entries": entries}}
    )
    return result.modified_count
...
def edit_task(username, date, task_index, updated_task):
    ...
    result = entries_collection.update_one(
        {"username": username, "date": date}, {"$set": {key: updated_task}}
    )
    return result.modified_count
...
def delete_task(username, date, task_index):
    ...
    result = entries_collection.update_one(
        {"username": username, "date": date}, {"$set": {"tasks": tasks}}
    )
    return result.modified_count
